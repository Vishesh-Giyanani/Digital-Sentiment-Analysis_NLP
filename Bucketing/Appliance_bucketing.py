...
def categorize_product(row, product_keywords):
    if check_keywords(row['Appliances-Text']):
        if row['Appliances_label'] == 'Positive':
            return 'positive'
        elif row['Appliances_label'] == 'Negative':
            return 'negative'
        else:
            return 'neutral'
    else:
...
def categorize_price(row1, price_keywords):
    if check_keywords_a(row1['Appliances-Text']):
        if row1['Appliances_label'] == 'Positive':
            return 'positive'
        elif row1['Appliances_label'] == 'Negative':
            return 'negative'
        else:
            return 'neutral'
    else:
...
df.to_csv('./Bucketing/Appliances_bucketing.csv', index=False)
