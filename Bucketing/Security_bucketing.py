...
df = pd.read_csv('./NLP/Security.csv')
...
def categorize_product(row, product_keywords):
    if check_keywords(row['Security-Text']):
        if row['Security_label'] == 'Positive':
            return 'positive'
        elif row['Security_label'] == 'Negative':
            return 'negative'
        else:
            return 'neutral'
    else:
...
def categorize_price(row1, price_keywords):
    if check_keywords_a(row1['Security-Text']):
        if row1['Security_label'] == 'Positive':
            return 'positive'
        elif row1['Security_label'] == 'Negative':
            return 'negative'
        else:
            return 'neutral'
    else:
...
df.to_csv('./Bucketing/Security_bucketing.csv', index=False)
