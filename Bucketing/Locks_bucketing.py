...
def categorize_product(row, product_keywords):
    if check_keywords(row['Locks-Text']):
        if row['Locks_label'] == 'Positive':
            return 'positive'
        elif row['Locks_label'] == 'Negative':
            return 'negative'
        else:
            return 'neutral'
    else:
...
def categorize_price(row1, price_keywords):
    if check_keywords_a(row1['Locks-Text']):
        if row1['Locks_label'] == 'Positive':
            return 'positive'
        elif row1['Locks_label'] == 'Negative':
            return 'negative'
        else:
            return 'neutral'
    else:
...
df.to_csv('./Bucketing/Locks_bucketing.csv', index=False)
